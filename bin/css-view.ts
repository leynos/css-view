...
async function main() {
	...
	program
		...
		.addOption(
			new Option("-b, --browser <browser>", "Playwright browser engine"),
		)
		.option(
			"--props <list>",
			"Comma or newline separated list of computed CSS properties",
		)
		.option(
			"--props-file <path>",
			"File with computed CSS properties, one per line",
		)
		.option(
			"--inherited <list>",
			"Walker-only: override inherited property list",
		)
		.option(
			"--inherited-file <path>",
			"Walker-only: provide inherited props via file",
		)
